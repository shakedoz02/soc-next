  ...
  return (
    <div className="relative w-64 h-64">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-slate-500 font-technical-mono text-sm tracking-widest uppercase">
...
function ActivityChart() {
  return (
    <div className="bg-[#1C2536] border border-[#1C2536] p-6 rounded-lg relative overflow-hidden group h-52">
      ...
      <div className="w-full h-24 relative">
        ...
        >
          ...
          <path
            d="M0 35 Q 10 20, 20 30 T 40 15 T 60 25 T 80 10 T 100 20"
            fill="none"
            stroke="#9FEF00"
            strokeWidth="1.5"
          ...
          />
          <path
            d="M0 35 Q 10 20, 20 30 T 40 15 T 60 25 T 80 10 T 100 20 V 40 H 0 Z"
...
export default function SummaryPage() {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(`Report ID: RPT-${Date.now()}`, pageWidth - 60, 285);
    ...
    <div className="p-8 custom-scrollbar overflow-y-auto font-assistant">
      ...
      <div className="mb-10 text-center">
        ...
        <p className="text-slate-400 text-lg max-w-2xl mx-auto">
          כל הכבוד, אנליסט. ביצועי המשמרת שלך היו מעל הממוצע הנדרש. הנתונים סונכרנו למסד הנתונים הראשי.
        </p>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6 items-start max-w-5xl mx-auto">
        ...
        {/* Score Ring - col-span-5 (RIGHT in RTL) */}
        <div className="col-span-12 lg:col-span-5 bg-[#1C2536] border border-[#1C2536] p-8 rounded-lg flex flex-col items-center justify-center min-h-[400px]">
          ...
          <div className="mt-8 flex gap-8">
            ...
            <div className="text-center">
              <div className="text-slate-500 text-[10px] font-technical-mono uppercase mb-1">Rank</div>
              <div className="text-[#9FEF00] font-technical-mono">TOP 12%</div>
            </div>
          </div>
        </div>

        {/* Stats + Activity Chart - col-span-7 (LEFT in RTL) */}
        <div className="col-span-12 lg:col-span-7 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={label} className="bg-[#1C2536] border border-[#1C2536] p-6 rounded-lg hover:border-[#9FEF00]/40 transition-all duration-300">
                <div className="flex items-center gap-2 text-slate-500 mb-2">
                  <span className="material-symbols-outlined text-sm">{icon}</span>
                  <span className="text-[10px] font-technical-mono uppercase tracking-wider">{label}</span>
                </div>
        ...
        <div className="col-span-12 bg-[#1C2536] border border-[#1C2536] p-6 rounded-lg flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <button
              ...
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-3 bg-[#9FEF00] text-[#111927] font-bold rounded hover:brightness-110 cyber-glow transition-all active:scale-95 duration-100"
            >
        ...
        <div className="col-span-12 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: 'description', label: 'פרוטוקול תחקיר', sub: 'פירוט מלא של כל שלבי החקירה שבוצעו במשמרת.', id: `DOC_ID: ${Math.floor(Math.random() * 9000 + 1000)}` },
            { icon: 'terminal', label: 'לוגים גולמיים', sub: 'היסטוריית פקודות מלאה שבוצעה בטרמינל האנליסט.', id: `LOG_ID: ${Math.floor(Math.random() * 9000 + 1000)}` },
            { icon: 'emergency', label: 'אירועים קריטיים', sub: 'סיכום אירועי האבטחה המרכזיים שטופלו.', id: `ALRT_ID: ${Math.floor(Math.random() * 9000 + 1000)}` },
            { icon: 'military_tech', label: 'עיטורי הצטיינות', sub: 'פירוט המדליות והבונוסים שנצברו במהלך הפעילות.', id: `ACHV_ID: ${Math.floor(Math.random() * 9000 + 1000)}` },
          ].map(({ icon, label, sub, id }) => (
            <div key={label} className="p-4 bg-[#111927] border border-[#1C2536] rounded group cursor-pointer hover:bg-[#1C2536] transition-colors">
              <div className="flex justify-between items-start mb-4">
                <span className="material-symbols-outlined text-[#9FEF00]">{icon}</span>
                <span className="text-[10px] text-slate-600 font-technical-mono">{id}</span>
              </div>
